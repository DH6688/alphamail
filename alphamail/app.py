...
def compose():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO inbox (sender, subject, message, email, time) VALUES (?,?,?,?,?)", sender, subject, message, address, time)
...
def inbox():
    ...
    if request.method == "POST":
        ...
        inbox = db.execute("SELECT * FROM inbox WHERE email = ?", email)
